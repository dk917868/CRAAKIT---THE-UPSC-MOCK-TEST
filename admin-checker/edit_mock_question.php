...
$res = mysqli_query($link, "select * from mock_questions_dummy where `question_no`='$qno' and `DateOfPaper`='$DateOfPaper'");
...
if (isset($_POST["submit1"])) {
	...
	mysqli_query($link, "UPDATE mock_questions_dummy SET `question`='$_POST[question]',`opt1`='$_POST[opt1]',`opt2`='$_POST[opt2]',`opt3`='$_POST[opt3]',`opt4`='$_POST[opt4]',`answer`='$_POST[answer]' WHERE DateOfPaper='$DateOfPaper' and question_no=$qno;" ) or die(mysqli_error($link));
...
?>
	<script type="text/javascript">
		...
		// history.back();
		window.location = "varifyquestions.php?DateOfPaper=<?php echo $DateOfPaper ?>";
	</script>
<?php
